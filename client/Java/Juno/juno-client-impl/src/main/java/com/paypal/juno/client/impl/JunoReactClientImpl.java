...
public class JunoReactClientImpl implements JunoReactClient {
     ...
     * @return Single<JunoResponse> - Emits a single response or Error with
     *         processing the request
     * @throws JunoException - Throws Exception if any exception while processing
     *                       the request
     */
     ...
     * @return Mono<JunoResponse> - Emits a single response or Error with processing
     *         the request
     * @throws JunoException - Throws Exception if any exception while processing
     *                       the request
     */
     ...
     * @return Mono<JunoResponse> - Emits a single response with the record value or
     *         Error
     * @throws JunoException - Throws Exception if any issue while processing the
     *                       request
     */
    ...
    /**
     * Update the record if present in Juno DB else create that record with the
    ...
    /**
     * Update the record if present in Juno DB and extend its TTL else create that
     * record with the supplied TTL.
     * 
     * @param key           - Key of the record to be Upserted
     * @param value         - Record Value
     * @param timeToLiveSec - Time to Live for the record
    ...
    /**
     * Compare the version of the record in Juno DB and update it only if the
     * supplied version
     * is greater than or equal to the existing version in Juno DB
     * 
     * @param jcx           - Juno record context
     * @param value         - Record Value
     * @param timeToLiveSec - Time to Live for the record. If set to 0 then the TTL
     *                      is not extended.
     * @return Mono<JunoResponse> - Emits a single response or Error with processing
     *         the request
     * @throws JunoException - Throws Exception if any exception while processing
    ...
    public Mono<JunoResponse> compareAndSet(RecordContext jcx, byte[] value, long timeToLiveSec) throws JunoException {
        try {
            ...
            final JunoRequest req = new JunoRequest(jcx.getKey(), value, jcx.getVersion(), timeToLiveSec,
                    jcx.getCreationTime(), JunoRequest.OperationType.Update);
            ...
            JunoMetrics.recordOpCount(socConfig.getJunoPool(), JunoRequest.OperationType.Update.getOpType(),
                    OperationStatus.IllegalArgument.getErrorText(), iae.getMessage());
            throw new JunoException(OperationStatus.IllegalArgument.getErrorText(), iae);
        } catch (Exception e) {
            throw new JunoException(e.getMessage());
        }
     ...
     *                Juno Request object
     * @return Observable<JunoResponse> - Emits a single response or Error with
     *         processing the request
     * @throws JunoException - Throws Exception if any exception while processing
     *                       the request
     */
    ...
    public Flux<JunoResponse> doBatch(final Iterable<JunoRequest> request) throws JunoException {
        ...
        if (request == null) {
            JunoMetrics.recordOpCount(socConfig.getJunoPool(), "JUNO_BATCH",
                    OperationStatus.IllegalArgument.getErrorText(), "null_request_list");
            throw new JunoException(OperationStatus.IllegalArgument.getErrorText(),
                    new IllegalArgumentException("Request argument is null"));
        } else if (!request.iterator().hasNext()) {
            JunoMetrics.recordOpCount(socConfig.getJunoPool(), "JUNO_BATCH",
                    OperationStatus.IllegalArgument.getErrorText(), "empty_request_list");
            throw new JunoException(OperationStatus.IllegalArgument.getErrorText(),
                    new IllegalArgumentException("Empty request list supplied"));
        }
        ...
        try {
            Flux<JunoResponse> resp = Flux.<JunoResponse>create(t -> {
                ...
                final ConcurrentHashMap<UUID, JunoMessage> reqIdReqMsgMap = JunoClientUtil.bulkValidate(request, t,
                        configHolder, socConfig, isAsync);
                System.out.println(reqIdReqMsgMap);
                ...
                try {
                    ...
                    while (retryCounter <= MAX_RETRY) {
                        ...
                        BlockingQueue<OperationMessage> respQueue = new ArrayBlockingQueue<OperationMessage>(
                                reqIdReqMsgMap.size());
                        ...
                        while (!reqSender.isDone() || reqSent.get() > resCount) {
                            ...
                            if (reqSent.get() == 0) {
                                ...
                                if ((System.currentTimeMillis() - start) > 50) {
                                    ...
                                    rootTrans.put("timeToSendRequest",
                                            String.valueOf(System.currentTimeMillis() - start));
                                    LOGGER.warn(JunoStatusCode.WARNING.toString() + " {} ", rootTrans);
                                    JunoMetrics.recordEventCount("JUNO_BATCH_SEND_DELAYED", "UNKNOWN",
                                            JunoMetrics.WARNING);
                                }
                            }
                            ...
                            JunoMessage respMessage = JunoClientUtil.decodeOperationMessage(responseOpeMsg,
                                    reqMsg.getKey(), configHolder);
                            JunoResponse junoResp = new JunoResponse(respMessage.getKey(), respMessage.getValue(),
                                    respMessage.getVersion(),
                                    respMessage.getTimeToLiveSec(), respMessage.getCreationTime(),
                                    respMessage.getStatus().getOperationStatus());
                            ...
                            if (respMessage.getStatus().getOperationStatus() != OperationStatus.Success) {
                                ...
                                if (operationRetry
                                        && JunoClientUtil.checkForRetry(respMessage.getStatus().getOperationStatus())) {
                                    JunoMetrics.recordOpCount(socConfig.getJunoPool(),
                                            "B_" + reqMsg.getOpType().getOpType(),
                                            respMessage.getStatus().getErrorText());
                                    continue;
                                }
                            }
                            ...
                            trans.put("USE_SSL", configHolder.getUseSSL() ? JunoConstants.JUNO_SSL_CLIENT
                                    : JunoConstants.JUNO_CLIENT);
                            ...
                            trans.put("msg_size", reqMsg.getMessageSize() + "|"
                                    + Long.toString(responseOpeMsg.getHeader().getMessageSize()));
                            ...
                            trans.put("server",
                                    responseOpeMsg.getServerIp() + ":" + String.valueOf(socConfig.getPort()));
                            ...
                            if (junoResp.getStatus().isTxnOk()) {
                                ...
                                JunoMetrics.recordOpTimer(JunoMetrics.JUNO_LATENCY_METRIC,
                                        "B_" + reqMsg.getOpType().getOpType(), socConfig.getJunoPool(),
                                        JunoMetrics.SUCCESS, duration);
                                ...
                                JunoMetrics.recordOpTimer(JunoMetrics.JUNO_LATENCY_METRIC,
                                        "B_" + reqMsg.getOpType().getOpType(), socConfig.getJunoPool(),
                                        JunoMetrics.ERROR, duration);
                            }
                            JunoMetrics.recordOpCount(socConfig.getJunoPool(), "B_" + reqMsg.getOpType().getOpType(),
                                    respMessage.getStatus().getErrorText());
                        ...
                        // remove the respQueue for the current batchOpaque. So that retry will use a
                        // different opaque.
                        ...
                        if (reqIdReqMsgMap.size() != 0) {
                            if (!operationRetry) {
                                ...
                                for (Map.Entry<UUID, JunoMessage> entry : reqIdReqMsgMap.entrySet()) {
                                    ...
                                    JunoResponse junoResp = new JunoResponse(jMsg.getKey(), jMsg.getValue(), 0, 0, 0,
                                            jMsg.getStatus().getOperationStatus());
                                    final Map<String, CharSequence> trans = new HashMap<String, CharSequence>();
                                    trans.put("isAsync", String.valueOf(isAsync));
                                    trans.put("USE_SSL", configHolder.getUseSSL() ? JunoConstants.JUNO_SSL_CLIENT
                                            : JunoConstants.JUNO_CLIENT);
                                    ...
                                    JunoMetrics.recordOpTimer(JunoMetrics.JUNO_LATENCY_METRIC,
                                            "B_" + jMsg.getOpType().getOpType(), socConfig.getJunoPool(),
                                            JunoMetrics.ERROR, duration);
                                    trans.put("Duration", String.valueOf(duration));
                                    LOGGER.info("TxnStatus : {}", trans);
                                    JunoMetrics.recordOpCount(socConfig.getJunoPool(),
                                            "B_" + jMsg.getOpType().getOpType(), junoResp.getStatus().getErrorText());
                                    t.next(junoResp);
                                }
                            } else {
                    ...
                    rootTrans.put("ROOT_TXN_STATUS",
                            txnFailed ? JunoStatusCode.ERROR.toString() : JunoStatusCode.SUCCESS.toString());
                    JunoMetrics.recordOpTimer(JunoMetrics.JUNO_LATENCY_METRIC, "JUNO_BATCH", socConfig.getJunoPool(),
                            txnFailed ? JunoMetrics.ERROR : JunoMetrics.SUCCESS,
    ...
    private Mono<JunoResponse> processSingle(final JunoRequest req, final JunoMessage.OperationType opType) {
        ...
        try {
            Mono<JunoResponse> resp = Mono.<JunoResponse>create(t -> {
                int opaque = 0;
                try {
                    final JunoMessage reqMsg = JunoClientUtil.validateInput(req, opType, configHolder); // TODO remove
                                                                                                        // optype
                    ...
                    while (retryCounter <= MAX_RETRY) {
                        ...
                        // wait for response to arrive
                        OperationMessage responseOpeMsg = respQueue.poll(configHolder.getResponseTimeout(),
                        ...
                        if (responseOpeMsg == null) {
                            ...
                            reqQueue.incrementFailedAttempts();
                            throw new JunoException(OperationStatus.ResponseTimeout.getErrorText(),
                                    new TimeoutException());
                        }
                        reqQueue.incrementSuccessfulAttempts();
                        // Decode the formed operation message and create the JunoResponse
                        JunoMessage respMsg = JunoClientUtil.decodeOperationMessage(responseOpeMsg, reqMsg.getKey(),
                                configHolder);
                        if (respMsg.getStatus().getOperationStatus() != OperationStatus.Success) {
                            if (operationRetry
                                    && JunoClientUtil.checkForRetry(respMsg.getStatus().getOperationStatus())) {
                                JunoMetrics.recordOpCount(socConfig.getJunoPool(), opType.getOpType(),
                                        respMsg.getStatus().getErrorText());
                                operationRetry = false;
                                // Retry intervl will be between 10 - 100 msecs
                                final int randVal = new Random().nextInt(MAX_RETRY_INTERVAL - MIN_RETRY_INTERVAL);
                                Thread.sleep(randVal + MIN_RETRY_INTERVAL);
                        ...
                        JunoResponse junoResp = new JunoResponse(reqMsg.getKey(), respMsg.getValue(),
                                respMsg.getVersion(),
                                respMsg.getTimeToLiveSec(), respMsg.getCreationTime(),
                                respMsg.getStatus().getOperationStatus());

                        // Check if we got the response only for our request by comparing the ID in
                        ...
                        // Return only on acceptable errors else throw exception. Set CAL ok if the
                        // error code is acceptable.
                        if (junoResp.getStatus() == OperationStatus.Success
                                || junoResp.getStatus() == OperationStatus.ConditionViolation ||
                                junoResp.getStatus() == OperationStatus.NoKey
                                || junoResp.getStatus() == OperationStatus.RecordLocked ||
                                junoResp.getStatus() == OperationStatus.UniqueKeyViolation
                                || junoResp.getStatus() == OperationStatus.TTLExtendFailure) {
                            trans.put("status", junoResp.getStatus().getErrorText());
                            LOGGER.info(JunoStatusCode.SUCCESS + " SUCCESS {} ", trans);
                            JunoMetrics.recordOpTimer(JunoMetrics.JUNO_LATENCY_METRIC, opType.getOpType(),
                                    socConfig.getJunoPool(), JunoMetrics.SUCCESS,
                                    System.currentTimeMillis() - opStartTime);
                            JunoMetrics.recordOpCount(socConfig.getJunoPool(), opType.getOpType(),
                                    junoResp.getStatus().getErrorText());
                            t.success(junoResp);
                        } else {
                            trans.put("server", socConfig.getJunoPool());
                    ...
                    JunoMetrics.recordOpTimer(JunoMetrics.JUNO_LATENCY_METRIC, opType.getOpType(),
                            socConfig.getJunoPool(), JunoMetrics.ERROR, System.currentTimeMillis() - opStartTime);
                    ...
                    if (iae.getCause() != null)
                        JunoMetrics.recordOpCount(socConfig.getJunoPool(), opType.getOpType(),
                                OperationStatus.IllegalArgument.getErrorText(), iae.getCause().getMessage());
                    else
                        JunoMetrics.recordOpCount(socConfig.getJunoPool(), opType.getOpType(),
                    ...
                    JunoMetrics.recordOpTimer(JunoMetrics.JUNO_LATENCY_METRIC, opType.getOpType(),
                            socConfig.getJunoPool(), JunoMetrics.ERROR, System.currentTimeMillis() - opStartTime);
                    JunoMetrics.recordOpCount(socConfig.getJunoPool(), opType.getOpType(),
                            OperationStatus.InternalError.getErrorText());
            ...
            LOGGER.error(JunoStatusCode.ERROR + " ERROR {} ", trans);
            JunoMetrics.recordOpTimer(JunoMetrics.JUNO_LATENCY_METRIC, opType.getOpType(), socConfig.getJunoPool(),
                    JunoMetrics.ERROR, System.currentTimeMillis() - opStartTime);
            JunoMetrics.recordOpCount(socConfig.getJunoPool(), opType.getOpType(),
                    OperationStatus.InternalError.getErrorText());
            LOGGER.error(e.getMessage() + " [" + this.configHolder.getProperties() + "]");
